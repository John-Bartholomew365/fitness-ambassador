...
const RegisterPage = () => {
  ...
  const handleSizeSelect = async (size: string) => {
    ...
    try {
      // Prepare vest data for API
      const vestData = {
        type: selectedVest.type,
        color: selectedVest.color,
        colorName: selectedVest.colorName,
        price: selectedVest.price,
        size: size,
      };

      // Show loading toast
      const loadingToast = toast.loading('Saving your vest selection...');

      // Submit to API
      const response = await fetch('/api/vest', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(vestData),
      });
      ...
      if (!response.ok) {
        ...
        toast.error(result.message || 'Failed to save vest selection');
        throw new Error(result.message || 'Failed to submit vest selection');
      }

      // Dismiss loading toast and show success
      toast.dismiss(loadingToast);
      toast.success('Vest selected successfully!');

      // Store the vestId in sessionStorage for the next page
      if (result.vestId) {
        sessionStorage.setItem('selectedVestId', result.vestId);
        // Also store the vest details in sessionStorage for display
        sessionStorage.setItem('selectedVestDetails', JSON.stringify({
          ...selectedVest,
          size: size
        }));
        ...
        throw new Error('No vestId received from server');
      }

    } catch (error) {
      console.error('Error submitting vest selection:', error);
      toast.error(
